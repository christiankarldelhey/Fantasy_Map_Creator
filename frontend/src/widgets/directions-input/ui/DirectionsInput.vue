<template>
  ...
  <div v-if="!mobile" class="directions-container absolute top-[100px] left-4 z-[9999] w-[400px] bg-parchment-base rounded-lg shadow-2xl border-2 border-gold flex flex-col overflow-hidden">
    <!-- Header -->
  ...
  <!-- Mobile: bottom sheet -->
  <BottomSheet v-else :expanded-vh="90" :collapsed-px="200">
    <template #default>
      <div class="flex flex-col h-full">
        <header class="flex items-center gap-3 px-4 py-3 border-b-2 border-earth-dark bg-parchment-light shrink-0">
          <button
            @click="handleBack"
            class="p-1 rounded-md hover:bg-parchment-dark transition-colors text-ink-brown hover:text-ink-black"
            title="Exit directions"
