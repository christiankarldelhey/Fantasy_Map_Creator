...
function findShortestPath(roads, startCoord, endCoord, config) {
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const mode = transport_mode || 'walk';
    const config = TRANSPORT_CONFIGS[mode] || TRANSPORT_CONFIGS.walk;

    // 1. Fetch all roads from database with their respective biome and altitude layers
    const roadsQuery = `
      SELECT 
        r.id, 
        r.name, 
